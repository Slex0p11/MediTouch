...
const MedicineOrdered = () => {
  ...
  const handleViewPrescription = (fileName) => {
    console.log("Prescription file name:", fileName); // Log to check the fileName value
  
    if (fileName) {
      // Navigate to the Prescription Viewer with the correct fileName
      navigate(`/prescriptions/${fileName}`);
    } else {
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {orders.map((order) => (
                <tr key={order.id} className="hover:bg-gray-50 transition-all duration-200 ease-in-out">
                  ...
                  <td className="px-6 py-4 font-semibold text-green-600">✅Completed</td>
                  <td className="px-6 py-4">
                    <button
                      onClick={() => handleViewPrescription(order.fileName)} // Pass the prescription file name here
